...
{
    // Convert arciris:// to arciris:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("arciris://"))
    {
        uri.replace(0, 12, "arciris:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Arciris.lnk";
}
...
{
    return GetAutostartDir() / "arciris.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Arciris-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  arciris-qt [" + tr("command-line options") + "]                     " + "\n";
